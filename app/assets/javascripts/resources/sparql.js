...
SPARQL = {
    ...
    download : {
        rdb : function(){
            var download_url = Utils.rdf2any.server+Utils.rdf2any.actions.convert+"rdb-converter.sql?dataset="+QueryBuilder.datasets.get_selected()+"&query="+encodeURIComponent(window.btoa($("#txt_sparql_query").val()));
        ...
        csv : function(){
            window.open(Utils.rdf2any.server+Utils.rdf2any.actions.convert+"csv-converter.csv?dataset="+QueryBuilder.datasets.get_selected()+"&query="+encodeURIComponent(window.btoa($("#txt_sparql_query").val())));
        },
        pdf : function(){
            Utils.alert("The PDF download feature has not been implemented yet.");
        },
        json : function(){
            window.open(Utils.rdf2any.server+Utils.rdf2any.actions.convert+"json?dataset="+QueryBuilder.datasets.get_selected()+"&query="+encodeURIComponent(window.btoa($("#txt_sparql_query").val())));
        }
    }
